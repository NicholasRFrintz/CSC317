...
function buildCardsUsingStrings(data) {
    return `<div class="product-card" id="${data.id}" onclick="fadeAndCount(id)">
...
function fetchPhotos(){
    ...
    .then((response) =>{
        ...
        document.getElementById('counter').innerHTML = "Photo Count: " + count;
    })
    .catch((error) =>{
        console.log(error);
    });
...
function fadeAndCount(id) {
  ...
  var remain = document.querySelectorAll(".product-card").length - 1;
  document.getElementById('counter').innerHTML = "Photo Count: " + remain;
  check(remain);
}
...
function check(remain) {
  if (remain = 0) {
    ...
    document.getElementById('remnant').innerHTML = "Only Blankness Remains...";
  }
}